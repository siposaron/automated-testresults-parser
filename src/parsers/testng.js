...
function getTestSuiteFromTest(rawTest, testCaseToGroupMap) {
  ...
  for (let i = 0; i < rawClasses.length; i++) {
    let testMethods = rawClasses[i]["test-method"].filter(
      (raw) => !raw["@_is-config"]
    );
  ...
  suite.passed = rawTestMethods.filter(
    (test) => test["@_status"] === "PASS"
  ).length;
  suite.failed = rawTestMethods.filter(
    (test) => test["@_status"] === "FAIL"
  ).length;
  suite.skipped = rawTestMethods.filter(
    (test) => test["@_status"] === "SKIP"
  ).length;
  const retried = rawTestMethods.filter(
    (test) => test["@_retried"] === true
  ).length;
  if (retried) {
...
function getTestSuite(rawSuite) {
  ...
  for (let i = 0; i < rawTests.length; i++) {
    ...
    for (let j = 0; j < rawClasses.length; j++) {
      let testMethods = rawClasses[j]["test-method"].filter(
        (raw) => !raw["@_is-config"]
      );
      testMethods.forEach((testMethod) => {
...
function getTestResult(json) {
  ...
  const suitesWithTests = suites.filter(
    (suite) => suite.test && suite["@_duration-ms"] > 0
  );
  ...
  if (suitesWithTests.length > 1) {
    ...
    for (let i = 0; i < rawTestsWithClasses.length; i++) {
      result.suites.push(
        getTestSuiteFromTest(rawTestsWithClasses[i], testCaseToGroupMap)
      );
    }
